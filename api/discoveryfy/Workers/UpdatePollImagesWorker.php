...
class UpdatePollImagesWorker extends BaseWorker
{
...
//            throw new InternalServerErrorException('The property spotify_playlist_uri is empty');
            $this->log('The property spotify_playlist_uri is empty');
            return $this->returnResult(false, $this->incrementAttempt($message), false);
        }

        // Create Playlist in Spotify
        $playlist = $this->getSpotifyApi()->getPlaylist($poll->get('spotify_playlist_uri'), ['fields' => 'images']);
        if (!$playlist) {
            $this->log(sprintf('Invalid playlist %s', $poll->get('spotify_playlist_uri')));
            return $this->returnResult(false, $this->incrementAttempt($message), false);
        }
