...
class RestartPollCreateSpotifyPlaylistHistoricWorker extends BaseWorker
{
...
//            throw new InternalServerErrorException('The property spotify_playlist_historic_uri is already set');
            $this->log('The property spotify_playlist_historic_uri is already set');
            return $this->returnResult(false, $this->incrementAttempt($message), false);
        }

        // Create Playlist in Spotify
        $playlist = $this->getSpotifyApi()->createPlaylist([
            'name' => $poll->getHistoricPlaylistName(),
            'description' => $poll->getHistoricPlaylistDescription(),
            'public' => true,
            'collaborative' => false,
        ]);

        // Update historic poll
        $poll->set('spotify_playlist_historic_uri', $playlist['id']);
